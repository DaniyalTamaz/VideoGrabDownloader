...
public class DownloadQueueService extends Service {
    ...
    public static void pause(Context c,String id){ DownloadTask t=DownloadTaskStore.get(c,id); if(t!=null){t.status=DownloadTask.STATUS_PAUSED; DownloadTaskStore.upsert(c,t); if(DownloadTask.KIND_DASH.equals(t.kind)) FFmpegKit.cancel();} }
    ...
    public static void cancel(Context c,String id){ DownloadTask t=DownloadTaskStore.get(c,id); if(t!=null){t.status=DownloadTask.STATUS_CANCELED;DownloadTaskStore.upsert(c,t);if(DownloadTask.KIND_DASH.equals(t.kind)) FFmpegKit.cancel();if(!t.tempPath.isEmpty())new File(t.tempPath).delete();} }
    ...
    private void dash(DownloadTask t)throws Exception{
        check(t.id);
        String mpd=HlsInspector.fetchText(t.url,t.cookie,t.userAgent,t.referer).toLowerCase(Locale.ROOT);
        if(mpd.contains("<contentprotection")||mpd.contains("urn:mpeg:cenc")||mpd.contains("widevine")||mpd.contains("playready"))throw new IOException("DRM/protected DASH is not supported");
        File file=temp(t,".dash.mp4"); if(file.exists()) file.delete();
        t.progress=8;DownloadTaskStore.upsert(this,t);notifyTask(t);
        String headers=DashInspector.headers(t.cookie,t.userAgent,t.referer);
        StringBuilder cmd=new StringBuilder("-hide_banner -loglevel error -y ");
        if(!headers.isEmpty())cmd.append("-headers ").append(DashInspector.q(headers)).append(' ');
        cmd.append("-i ").append(DashInspector.q(t.url)).append(' ');
        if(DownloadTask.MODE_MP3.equals(t.mode)){
            if(t.dashAudioStream<0)throw new IOException("No audio track found in DASH manifest");
            cmd.append("-map 0:").append(t.dashAudioStream).append(" -vn -c:a copy ").append(DashInspector.q(file.getAbsolutePath()));
        }else{
            if(t.dashVideoStream<0)throw new IOException("No video track selected");
            cmd.append("-map 0:").append(t.dashVideoStream).append(' ');
            if(t.dashAudioStream>=0)cmd.append("-map 0:").append(t.dashAudioStream).append(' ');
            cmd.append("-c copy -movflags +faststart ").append(DashInspector.q(file.getAbsolutePath()));
        }
        t.progress=20;DownloadTaskStore.upsert(this,t);notifyTask(t);
        FFmpegSession session=FFmpegKit.execute(cmd.toString());
        DownloadTask state=DownloadTaskStore.get(this,t.id);
        if(state==null||DownloadTask.STATUS_CANCELED.equals(state.status))throw new Canceled();
        if(DownloadTask.STATUS_PAUSED.equals(state.status))throw new Paused();
        if(!ReturnCode.isSuccess(session.getReturnCode())){
            String logs=session.getAllLogsAsString(); if(logs!=null&&logs.length()>220)logs=logs.substring(logs.length()-220);
            throw new IOException("DASH merge failed"+(logs==null||logs.trim().isEmpty()?"":": "+logs.trim()));
        }
        t.progress=92;t.mime="video/mp4";DownloadTaskStore.upsert(this,t);notifyTask(t);
        finish(t,file,MediaPublisher.stripExtension(t.name)+".mp4");
    }
